(function (global, factory) {
    ...
    Object.assign(ImageProcesser.prototype, {
        ...
        apply_kernel: function (border = 'icrop') {
            ...
            if (border === 'extend') {
                const extended = nj.zeros([this.height + filter_padding, this.width + filter_padding], 'uint8');
                for (var y = 0; y < this.height + filter_padding; y++) {
                    for (var x = 0; x < this.width + filter_padding; x++) {
                        ...
                        extended.set(y, x, this.img.get(original_y, original_x));
                    }
                }
                this.img = extended;
            ...
            const new_img = nj.zeros([this.height - filter_padding, this.width - filter_padding], 'uint8');
                    ...
                    if (this.kernel === 'box') {
                        ...
                        for (var i = 0; i < filter_size; i++) {
                            for (var j = 0; j < filter_size; j++) {
                                result = result + filter.get(i, j) / 9;
                            }
                        }
        ...
        apply_xform: function () {
            ...
            const new_img = nj.zeros([3 * this.height, 3 * this.width], 'uint8');
            ...
            for (var y = -this.height; y < 2 * this.height; y++) {
                for (var x = -this.width; x < 2 * this.width; x++) {
                    ...
                    if (original_x < 0 || original_x >= this.width || original_y < 0 || original_y >= this.height) {
                        ...
                        new_img.set(y + this.height, x + this.width, result);
                    }
                }
            }
